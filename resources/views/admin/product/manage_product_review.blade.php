...
@section('admin_layout')
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-lg-6 col-xl-5 col-xxl-4" id="cart_content">
            <div class="card">
                @include('admin.product.admincart')
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#search_product').on('keyup', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('product.review.search') }}',
            ...
            $(document).on('click', '#btn-add-to-cart', function() {
                ...
                $.ajax({
                    url: '{{ route('product.review.insert_item_order') }}',
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        $('#notification').html('<div class="alert alert-success">Sản phẩm đã được thêm vào giỏ hàng.</div>');
                        swal("Thành công", "{{ session('success') }}", "success");
                        loadCartContent();
                    },
                    error: function(xhr) {
                        $('#notification').html('<div class="alert alert-danger">Lỗi khi thêm sản phẩm vào giỏ hàng.</div>');
                        console.log('Error:', xhr.responseText);
                    }
                });
            });
